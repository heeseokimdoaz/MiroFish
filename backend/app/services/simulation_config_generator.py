...
**시드 기반 엄격 규칙 (가장 중요 — 절대 위반 금지)**:
- 초기 게시글 내용은 반드시 시드 데이터에 실제로 명시된 사건·의혹·논란·인물만 언급
- 시드에 없는 가상의 스캔들, 허위 사건, 존재하지 않는 정책을 만들어내지 마세요
- 시드에 "검증 필요 정보" 또는 "의혹·논란"으로 제시된 사안이 있으면, 해당 진영이 공격하거나 방어하는 반응을 반드시 게시글로 포함시키세요
- 시드에 "커뮤니티 핫 이슈", "실시간 검색 트렌드", "AI 정국 브리핑"에 나온 주제를 우선적으로 활용하세요
- 시드에 있는 실제 인물 이름과 실제 발언 맥락만 사용 (예: "한채훈 의왕시의원 강제추행 혐의", "광주 광산 을 선거구 획정 혼선" 같은 실제 이슈)

**초기 게시글 작성 규칙**:
- 실제 한국 SNS/커뮤니티(트위터, 커뮤니티 게시판, 유튜브 댓글)의 말투 사용
- 설문조사나 교과서 같은 정중한 질문 금지
- 각 Agent의 성격과 입장에 맞는 감정적·편향적 의견 표현
- 뉴스 기관은 헤드라인 스타일, 일반인은 반말/구어체, 정치인은 공식 성명 스타일
- 시드에 있는 이슈를 다양한 각도로 다루되, 사실 관계는 변형하지 마세요

**정당 간 관계 규칙 (절대 위반 금지)**:
- 같은 정당 소속은 서로 공격하지 않음 (민주당↔민주당 우호, 국힘↔국힘 우호)
- 민주당 Agent가 민주당 후보를 비판하는 게시글을 만들지 마세요
- 국민의힘 Agent가 국민의힘 후보를 비판하는 게시글을 만들지 마세요
- 비판은 반드시 상대 진영 또는 시드에 나온 의혹 대상만
...
- **언론** (MediaOutlet): 활성도 중간(0.4-0.6), 전일 활동(7-23), 응답 빠름(5-30분), 영향력 높음(2.0-2.5), stance는 반드시 "neutral"
...
## 정당 소속에 따른 stance/sentiment 규칙 (매우 중요!)
- **같은 정당 후보/지지자끼리는 절대 공격하지 않습니다**
- 더불어민주당 소속 Agent → 민주당 후보에게 supportive (sentiment_bias: 0.3~0.8), 국민의힘 후보에게 opposing (sentiment_bias: -0.3~-0.8)
- 국민의힘 소속 Agent → 국민의힘 후보에게 supportive (sentiment_bias: 0.3~0.8), 민주당 후보에게 opposing (sentiment_bias: -0.3~-0.8)
- 무소속/일반 시민 → neutral 또는 개인 성향에 따라 자유롭게
- stance가 "supportive"인 Agent는 자기 진영을 지지하고 상대 진영을 비판합니다
- stance가 "opposing"인 Agent는 상대 진영을 비판합니다
- **민주당이 민주당을 공격하거나, 국민의힘이 국민의힘을 공격하는 설정은 절대 만들지 마세요**
