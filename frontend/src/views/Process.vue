            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  문서 업로드 후, LLM이 문서 내용을 분석하여 여론 시뮬레이션에 적합한 온톨로지 구조(엔티티 타입 + 관계 타입)를 자동 생성
                </div>
              </div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">생성된 엔티티 타입 ({{ projectData.ontology.entity_types?.length || 0 }})</div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">생성된 관계 타입 ({{ projectData.ontology.relation_types?.length || 0 }})</div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">API 설명</div>
                <div class="detail-content">
                  생성된 온톨로지를 기반으로, 문서를 청크 분할 후 Zep API를 호출하여 지식 그래프를 구축하고, 엔티티와 관계를 추출
                </div>
              </div>
...
const handleNewProject = async () => {
  ...
  if (!pending.isPending || pending.files.length === 0) {
    error.value = '업로드 대기 파일이 없습니다. 홈으로 돌아가서 다시 시도해 주세요'
    loading.value = false
    return
  }
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      buildProgress.value.message = '그래프 구축 태스크 시작됨...'

      // 폴링을 위해 task_id 저장
      const taskId = response.data.task_id
      
      // 그래프 데이터 폴링 시작 (태스크 상태 폴링과 독립적)
      startGraphPolling()
      ...
      if (task.status === 'completed') {
        ...
        if (projectResponse.success) {
          ...
          if (projectResponse.data.graph_id) {
            console.log('📊 전체 그래프 로딩:', projectResponse.data.graph_id)
            await loadGraph(projectResponse.data.graph_id)
            console.log('✅ 그래프 로딩 완료')
          }
        }
