...
const showEdgeLabels = ref(true) // 기본 엣지 레이블 표시
...
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // 시뮬레이션 상태에서 비시뮬레이션 상태로 전환, 종료 안내 표시
...
const renderGraph = () => {
  ...
  const edgePairCount = {}
  const selfLoopEdges = {} // 노드별 셀프루프 엣지 그���
  const tempEdges = edgesData
    .filter(e => nodeIds.has(e.source_node_uuid) && nodeIds.has(e.target_node_uuid))
  
  // 노드 쌍 간 엣지 수량 통계, 셀프루프 엣지 수집
  tempEdges.forEach(e => {
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      ...
      edges.push({
        ...
        rawData: {
          ...
          selfLoopEdges: allSelfLoops // 모든 셀프루프 엣지 상세 정보 저장
        }
      })
      return
    }
    ...
    // 엣지 방향이 정규화된 방향과 일치하는지 판단 (소스UUID < 타겟UUID)
    ...
    if (totalCount > 1) {
      ...
      // 엣지 방향이 정규화 방향과 반대면 곡률 반전
      // 모든 엣지가 동일 기준에서 분포하여 방향 차이로 인한 겹침 방지
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // 노드 쌍 간 엣지 수에 따라 거리 동적 조정
      // 기본 거리 150, 엣지당 40씩 증가
    ...
    // 중심 방향 인력 추가, 독립 노드 그룹을 중심 영역으로 모으기
  ...
  const getLinkPath = (d) => {
    ...
    // 연결선 방향에 수직인 오프셋, 거리 비율로 계산하여 곡선 가시성 보장
    // 엣지가 많을수록 거리 대비 오프셋 비율 증가
    const pairTotal = d.pairTotal || 1
    const offsetRatio = 0.25 + pairTotal * 0.05 // 기본 25%, 엣지당 5%씩 증가
  ...
  const getLinkMidpoint = (d) => {
    ...
    // 2차 베지에 곡선 공식 B(t) = (1-t)^2*P0 + 2(1-t)t*P1 + t^2*P2, t=0.5
  ...
  simulation.on('tick', () => {
    ...
    // 엣지 레이블 위치 업데이트 (회전 없이 수평 표시로 가독성 향상)
