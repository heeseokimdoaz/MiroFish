          ...
          <div class="action-bar-tabs">
            ...
            <div class="agent-dropdown" v-if="profiles.length > 0">
              ...
              >
                ...
                <span>{{ selectedAgent ? selectedAgent.name : '시뮬레이션 세계의 누구와도 대화' }}</span>
              ...
              <div v-if="showAgentDropdown" class="dropdown-menu">
                ...
                >
                  <div class="agent-avatar">{{ (agent.name || 'A')[0] }}</div>
        ...
        <div v-if="activeTab === 'chat'" class="chat-container">
          ...
          <div v-if="chatTarget === 'report_agent'" class="report-agent-tools-card">
            <div class="tools-card-header">
              ...
              <div class="tools-card-info">
                ...
                <div class="tools-card-subtitle">리포트 생성 에이전트의 빠른 대화 버전, 4종 전문 도구 호출 가능, MiroFish의 전체 메모리 보유</div>
              </div>
              ...
              <div class="tools-grid">
                ...
                <div class="tool-item tool-green">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">자율형 인터뷰로, 시뮬레이션 세계의 개체와 병렬로 다회차 대화를 수행하여 비정형 관점 데이터와 심리 상태를 수집</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div v-if="chatTarget === 'agent' && selectedAgent" class="agent-profile-card">
            <div class="profile-card-header">
              <div class="profile-card-avatar">{{ (selectedAgent.name || 'A')[0] }}</div>
              <div class="profile-card-info">
                <div class="profile-card-name">{{ selectedAgent.name }}</div>
          ...
          <div class="chat-messages" ref="chatMessages">
            <div v-if="chatHistory.length === 0" class="chat-empty">
              ...
              <p class="empty-text">
                {{ chatTarget === 'report_agent' ? 'Report Agent와 대화하여 리포트 내용을 심층적으로 살펴보세요' : '시뮬레이션 개체와 대화하여 그들의 관점을 알아보세요' }}
              </p>
            </div>
            <div 
            ...
            >
              <div class="message-avatar">
                ...
                <span v-else>{{ msg.role === 'assistant' && chatTarget === 'report_agent' ? 'R' : (selectedAgent?.name?.[0] || 'A') }}</span>
              </div>
              <div class="message-content">
                <div class="message-header">
                  <span class="sender-name">
                    {{ msg.role === 'user' ? 'You' : (chatTarget === 'report_agent' ? 'Report Agent' : (selectedAgent?.name || 'Agent')) }}
                  </span>
            ...
            <div v-if="isSending" class="chat-message assistant">
              <div class="message-avatar">
                <span>{{ chatTarget === 'report_agent' ? 'R' : (selectedAgent?.name?.[0] || 'A') }}</span>
              </div>
              <div class="message-content">
            ...
            <div class="setup-section">
              ...
              <div class="agents-grid">
                ...
                >
                  ...
                  >
                  <div class="checkbox-avatar">{{ (agent.name || 'A')[0] }}</div>
                  <div class="checkbox-info">
                    <span class="checkbox-name">{{ agent.name }}</span>
...
const chatHistoryCache = ref({}) // 모든 대화 기록 캐��: { 'report_agent': [], 'agent_0': [], 'agent_1': [], ... }
...
const renderMarkdown = (content) => {
  ...
  // <p><br>가 블록 요소 바로 앞에 오는 경우 정리 (불필요한 빈 줄로 인한)
  ...
  // 비연속 순서 목록 번호 수정: 단일 항목 <ol>이 단락 내용으로 분리될 때 번호 연속 유지
...
const sendToAgent = async (message) => {
  ...
  addLog(`${selectedAgent.value.name}에게 전송: ${message.substring(0, 50)}...`)
  ...
  if (res.success && res.data) {
    ...
    if (responseContent) {
      ...
      addLog(`${selectedAgent.value.name} 응답 완료`)
    } else {
...
const submitSurvey = async () => {
  ...
  addLog(`${selectedAgents.value.size}명 대상에게 설문 전송 중...`)
    ...
    if (res.success && res.data) {
      ...
      for (const interview of interviews) {
        ...
        surveyResultsList.push({
          agent_id: agentIdx,
          agent_name: agent?.name || agent?.username || `Agent ${agentIdx}`,
...
const loadReportData = async () => {
  ...
  try {
    addLog(`리포트 데이터 로딩: ${props.reportId}`)
...
const loadAgentLogs = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog('리포트 데이터 로딩 완료')
    }
  } catch (err) {
    addLog(`리포트 로그 로딩 실패: ${err.message}`)
  }
}
...
const loadProfiles = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`${profiles.value.length}개 시뮬레이션 개체 로딩 완료`)
    }
  } catch (err) {
    addLog(`시뮬레이션 개체 로딩 실패: ${err.message}`)
  }
}
...
/* Left Panel - Report Style (Step4Report.vue와 동일) */
.left-panel.report-style {
  width: 45%;
  min-width: 450px;
  background: #FFFFFF;
...
/* 순서 목록 번호 수정 - CSS 카운터로 여러 ol 연속 번호 매��기 */
.message-text {
