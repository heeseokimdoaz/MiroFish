          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <!-- 범용 폴백: 알 수 없는 타입이거나 content가 있지만 위에서 처리되지 않은 경우 -->
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 시뮬레이션 완료 여부 확인 (runner_status 또는 플랫폼 완료 상태로 판단)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // 추가 확인: 백엔드가 아직 runner_status를 업데이트하지 못했지만 플랫폼이 완료를 보고한 경우
      // twitter_completed 와 reddit_completed 상태로 판단
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 자동 스크롤하지 않고 사용자가 자유롭게 타임라인을 볼 수 있도록 함
      // 새 액션은 하단에 추가됨
    }
  } catch (err) {
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('리포트 생성 요청 전송 완료, 잠시 기다려주세요...')
    return
  }
