    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 0, 'completed': phase > 0 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            simulation 인스턴스를 생성하고 시뮬레이션 세계 파라미터 템플릿을 불러옵니다
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            컨텍스트를 결합하여 지식 그래프에서 엔티티와 관계를 자동으로 추출하고, 시뮬레이션 개체를 초기화하며 현실 시드를 기반으로 고유한 행동과 기억을 부여합니다
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            LLM이 시뮬레이션 요구사항과 현실 시드를 기반으로 세계 시간 흐름, 추천 알고리즘, 각 개체의 활동 시간대 등의 파라미터를 지능적으로 설정합니다
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <p class="description">시뮬레이션 환경 준비 완료. 시뮬레이션을 시작할 수 있습니다</p>
          ...
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                ...
                <span class="section-desc">MiroFish가 자동으로 현실 <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> 시간 계획을 수립합니다. 각 라운드는 현실 <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> 분의 시간 흐름을 나타냅니다</span>
              </div>
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                ...
                <div class="range-wrapper">
                  <input
                    ...
                    :style="{ '--percent': ((customMaxRounds - 3) / (autoGeneratedRounds - 3)) * 100 + '%' }"
                  />
                  <div class="range-marks">
                    <span>3</span>
                    <span
                      class="mark-recommend"
                      :class="{ active: customMaxRounds === 5 }"
                      @click="customMaxRounds = 5"
                      :style="{ position: 'absolute', left: `calc(${(5 - 3) / (autoGeneratedRounds - 3) * 100}% - 30px)` }"
                    >5 (권장 — 10분 이내)</span>
              ...
              <div v-else class="rounds-content auto" key="auto">
                <div class="auto-info-card">
                  ...
                  <div class="auto-content">
                    ...
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">처음 실행하는 경우, ‘사용자 지정 모드’로 전환하여 시뮬레이션 라운드를 줄이고 빠르게 효과를 확인하고 오류 위험을 줄이는 것을 강력히 권장합니다 ➝</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div class="modal-section" v-if="selectedProfile.persona">
            ...
            <div class="persona-dimensions">
              ...
              <div class="dimension-card">
                ...
                <span class="dim-desc">개인 연결 및 상호작용 그래프</span>
              </div>
            </div>
...
// 로그 중복 제거: 마지막 출력 핵심 정보 기록
let lastLoggedMessage = ''
let lastLoggedProfileCount = 0
let lastLoggedConfigStage = ''

// 시뮬레이션 라운드 설정 (10분 이내 완료 목표)
const useCustomRounds = ref(true)  // 기본: 사용자 지정으로 빠른 실행
const customMaxRounds = ref(5)     // 기본 5라운드 (전체 flow 10분 이내)
...
watch(currentStage, (newStage) => {
  if (newStage === '生成Agent人设' || newStage === 'generating_profiles') {
    ...
    phase.value = 2 // 설정 단계에 해당
  }
})

// 설정에서 자동 생성된 라운드 수 계산 (하드코딩 기본값 사용하지 않음)
const autoGeneratedRounds = computed(() => {
  ...
  // 최대 라운드 수가 40(권장값) 미만이 되지 않도록 보장, 슬라이더 범위 이상 방지
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    ...
    // 자동 생성 라운드 유지, max_rounds 파라미터 미전달
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (res.data.expected_entities_count) {
        ...
        addLog(`Zep 그래프에서 읽어온 ${res.data.expected_entities_count} 개 엔티티`)
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // API에서 유효한 값 반환 시에만 업데이트, 기존 유효값 덮어쓰기 방지
    ...
    if (res.success && res.data) {
      ...
      if (data.config_generated && data.config) {
        ...
        if (data.summary) {
          ...
          addLog(`  ├─ 초기 게시글: ${data.summary.initial_posts_count}개`)
...
const loadPreparedData = async () => {
  ...
  addLog(`${profiles.value.length}개 Agent 페르소나 로딩 완료`)
